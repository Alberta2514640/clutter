...
variable "ansible_ssh_private_key" {
  description = "SSH private key PEM content used by the Ansible runner for direct SSH transport"
  type        = string
  sensitive   = true
}
variable "ansible_ssh_known_hosts" {
  description = "known_hosts entries used by the Ansible runner for strict SSH host verification"
  type        = string
  sensitive   = true
}
