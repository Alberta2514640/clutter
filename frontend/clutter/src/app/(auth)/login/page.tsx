...
//   credential: string; // actual JWT token from Google (what we send to the backend).
//   select_by: string; // how the credential was selected (button, one-tap, etc.).
//   clientId?: string;
// };
...
// // TODO: later import from config/env
...
//   // 1) Called by Google when the user successfully logs in
...
//         const data: { token: string; userData: unknown } = await res.json();
...
//       } catch (err) {
//         console.error(err);
//         setErrorMsg("Something went wrong. Please try again. yerba matte");
//       } finally {
//         setIsLoading(false);
//       }
//     },
...
//   // 2) Initialize Google Identity Services when script is ready
//   useEffect(() => {
//     if (!GOOGLE_CLIENT_ID) {
...
//     // if not yet available, poll until the script has loaded
//     if (!gsiReady) {
...
//   // 3) Trigger Google login prompt when the user clicks the button
...
//             {/* Google Login Button (calls your Lambda under the hood) */}
